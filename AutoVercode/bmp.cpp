...
#include <windows.h>
#include <stdio.h>
double FXZOOMRATIO = 0.5;   //x轴放大倍数
double FYZOOMRATIO = 0.5;   //y轴放大倍数
...
bool readBmp(const wchar_t* bmpName)
{
	FILE *fp;
	_wfopen_s(&fp, bmpName, TEXT("rb"));
	if(fp==0)
	{
...
bool saveBmp(const wchar_t* bmpName,unsigned char *imgBuf,int width,int height,int biBitCount,RGBQUAD *pColorTable)
{
...
bool bmpzoom(const wchar_t* szSrcBmp, const wchar_t* szDstBmp, long newWidth, long newHeight)
{
	readBmp(szSrcBmp);

	newBmpWidth = newWidth; 
	newBmpHeight = newHeight;

	FXZOOMRATIO = (newBmpWidth - 0.5) / bmpWidth;
	FYZOOMRATIO = (newBmpHeight - 0.5) / bmpHeight;
